...
class StockSerial(models.TransientModel):
    ...
    def fields_view_get(self, view_id=None, view_type="form", toolbar=False, submenu=False):
        ...
        if "serial_ids" in res["fields"]:
            ...
            doc = etree.XML(arch)
            if context.get("active_model") == "stock.move" and context.get("active_id"):
                ...
                for node in doc.xpath("//field[@name='lot_id']"):
                    ...
                    if move.picking_type_id.use_existing_lots:
                        ...
                        sub_domain = (
                            "('product_id', '=?', " + "product_id)," " ('last_location_id', '=', " + "source_loc_id)"
                        )
                    else:
                        node = apply_modifiers(node, {"readonly": True})

                    if sub_domain:
                        new_domain = domain_str_append(node.get("domain"), sub_domain)
                        node.set("domain", new_domain)
                res["fields"]["serial_ids"]["views"]["tree"]["arch"] = etree.tostring(doc)
        return res
    ...
    def default_get(self, field_list):
        ...
        res["product_qty"] = sum(
            move_id.picking_id.move_lines.filtered(lambda mov: mov.product_id.id == product_id).mapped(
                "product_uom_qty"
            )
        )
        return res
                ...
                if serial_id.lot_id in serial:
                    return {
                        "warning": {
                            "title": _("Warning"),
                            "message": (
                                _("The Serial number %s already captured") % (serial_id.lot_id.name.encode("utf-8"))
                            ),
                        }
                    }
    ...
    def move_serial(self):
        ...
        for move in self:
            ...
            move_id.picking_id.pack_operation_ids.filtered(lambda dat: dat.product_id.id == product_id).unlink()
            ...
            if len(move.serial_ids) > total_product_uom_qty:
                raise ValidationError(
                    _("The serial numbers loaded cannot be greater" " than the requested quantity of the product")
                )
            ...
            for move_serial in move.serial_ids:
                ...
                if move_id.picking_type_id.use_create_lots:
                    if quant_obj.search_count(
                        ...
                        raise ValidationError(_("The serial number %s is already stock") % (move_serial.serial))
                    self._get_pack_ops_lot(move, move_serial)
                elif all(
                    [move_id.picking_type_id.use_existing_lots, move_id.picking_id.picking_type_id.code == "incoming"]
                ):
                    if quant_obj.search_count(
                        [
                            ("product_id", "=", move_id.product_id.id),
                    ...
                    raise ValidationError(_("Serial number %s not found") % (move_serial.serial))
                elif move_serial.lot_id.quant_ids.filtered("reservation_id"):
                    raise ValidationError(
                        _("The serial number %s is already reserved in" " another move") % (move_serial.serial)
                    )
                else:
...
class StockSerialLine(models.TransientModel):
    ...
    def _compute_default_partner_id(self):
        move = "move_id" in self._context and self.env["stock.move"].browse(self._context["move_id"]) or False
    ...
    product_id = fields.Many2one("product.product", default=_compute_default_partner_id)
    source_loc_id = fields.Many2one("stock.location", default=_compute_default_location_id)
            ...
            if move_id.picking_id.picking_type_id.code == "incoming":
                ...
                if other_quants:
                    return {
                        "warning": {
                            "title": _("Warning"),
                            "message": (_("The serial number %s is already in stock") % (serial_number)),
                        }
                    }

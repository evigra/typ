...
class StockTransferDetails(models.TransientModel):
    ...
    def default_get(self, values):
        ...
        for item in res.get("item_ids", []):
            item["supplier_code"] = self.get_supplier_code(picking, item["product_id"])
    ...
    def get_supplier_code(self, picking, product_id):
        suppliers = (
            self.env["product.product"].browse(product_id).seller_ids.filtered(lambda r: r.name == picking.partner_id)
        )

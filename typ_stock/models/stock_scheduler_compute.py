...
class StockSchedulerCompute(models.TransientModel):
    ...
    warehouse_id = fields.Many2one(
        "stock.warehouse", string="Warehouse", help="select warehouse" " to running schedulers"
    )
    ...
    def procure_calculation(self):
        ...
        context = dict(self._context, warehouse_id=self.warehouse_id, importance=self.importance, group_id=group_id.id)
...
class ProcurementGroup(models.Model):
    ...
    def _get_orderpoint_domain(self, company_id=False):
        ...
        if self._context.get("warehouse_id"):
            ctx.setdefault("order_point_domain", []).append(("warehouse_id", "=", self._context["warehouse_id"].id))
        if self._context.get("importance"):
            ctx.setdefault("order_point_domain", []).append(("importance", "=", self._context["importance"]))

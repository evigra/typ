    ...
    def action_cancel(self):
        ...
        if (not cancel_picking and purchase_ids and (self.mapped(
            'location_dest_id').filtered(
                lambda pick: pick.usage != 'customer') or len(self) > 1) and
                self.filtered(
        ...
        move_origns = move_model.search([
            ('move_dest_ids', 'in', moves_with_transit_loc.ids),
            ('state', '!=', 'done')])
    ...
    def write(self, vals):
        ...
        for move_line_id in self.mapped('picking_id.move_line_ids').filtered(
            ...
            quant_obj._update_reserved_quantity(
                move_line_id.product_id, move_line_id.location_id, 0,
                lot_id=move_line_id.lot_id, strict=True)
            quant_obj._update_reserved_quantity(
                move_line_id.product_id, move_line_id.location_id, 1,
                lot_id=move_line_id.lot_id, strict=True)

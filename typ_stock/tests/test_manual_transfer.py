...
class TestManualTransfer(TestTypStock):
    ...
    def test_10_make_transfer(self):
        ...
        manual_transfer = self.env['stock.manual_transfer'].sudo(
            demo_user).create(
            {
        ...
        picking_ids = manual_transfer.env['stock.picking'].search(
            [('group_id', '=', procurement_group_id)])
        self.assertTrue(len(picking_ids) == 2,
                        'Pickings were not created correctly')

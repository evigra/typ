...
class TestTypStock(common.TransactionCase):
    def setUp(self):
        ...
        self.group_cancel_picking = self.env.ref("typ_stock.group_cancel_picking_with_move_not_in_transit_loc")
        ...
        self.first_pick = [pick for pick in self.sale_order.picking_ids if pick.state == "confirmed"][0]
    ...
    def update_product_qty(self):
        self.lot_1 = self.lot.create({"name": "lot_1", "product_id": self.product_test.id})
        self.lot_2 = self.lot.create({"name": "lot_2", "product_id": self.product_test.id})

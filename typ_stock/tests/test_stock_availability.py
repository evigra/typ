...
class TestStockAvailability(TransactionCase):
    def setUp(self):
        ...
        self.product_deluxe = self.env.ref("typ_stock.product_product_whead_deluxe")
        ...
        self.sale_team = self.env.ref("default_warehouse_from_sale_team.section_sales_default_team")
        self.env.ref("base.user_demo").write(
            {"sale_team_ids": [(6, 0, [self.sale_team.id])], "default_section_id": self.sale_team.id}
        )
    ...
    def test_10_validation_not_allow_negative_numbers(self):
        ...
        pickings = sale.picking_ids.filtered(lambda picking: "OUT" in picking.name)
        ...
        msg = "Negative Quant creation error of the product %s. " "Contact Vauxoo personnel immediately" % (
            self.product.name
        )
    ...
    def test_30_validation_return_customer_amount(self):
        ...
        # Start return
        wizard_return_id = self.return_obj.with_context(context).sudo(demo_user).create({})

        # Return 10 products
        wizard_return_id.product_return_moves[0].write({"quantity": 10})

        msg = "The return of the product %s, exceeds the amount invoiced" % (self.product.name)
        with self.assertRaisesRegexp(UserError, msg):
    ...
    def create_picking(self, picking_type, source_location, dest_location, qty):
    ...
    def test_50_validate_quant_reservation(self):
        ...
        value = self.transfer_obj.with_context(
            {"active_model": "stock.picking", "active_id": pick.id, "active_ids": [pick.id]}
        ).default_get([])
        line = []
        ...
        # Check qty
        vqty = v_qty + 100
        self.assertEqual(vqty, self.product.virtual_available, "The virtual available is wrong")
        ...
        # Check qty
        self.assertEqual(10, self.product.virtual_available, "The virtual available is wrong")
        # Create picking with more thant available
        pick2 = self.create_picking(ptype, source, dest, 10)

        value = self.transfer_obj.with_context(
            {"active_model": "stock.picking", "active_id": pick2.id, "active_ids": [pick2.id]}
        ).default_get([])
        line = []
        for ope in value.get("item_ids", []):
        ...
        value = self.transfer_obj.with_context(
            {"active_model": "stock.picking", "active_id": pick1.id, "active_ids": [pick1.id]}
        ).default_get([])
        line = []
        for ope in value.get("item_ids", []):
            line.append((0, 0, ope))
    ...
    def test_60_validation_return_customer_distinct_sale(self):
        ...
        pickings_returned = sale.picking_ids.filtered(lambda picking: "IN" in picking.name)
        ...
        pickings_second = sale_second.picking_ids.filtered(lambda picking: "OUT" in picking.name)
        ...
        wizard_return_id.product_return_moves.filtered(lambda dat: dat.product_id == self.product_deluxe).unlink()

...
class TestReorderingRule(TestTypStock):
    ...
    def get_threads(self):
        return [
            ...
            if thread.name.startswith("Thread-") and thread.name not in self.thread_excl
        ]
    ...
    def write_data(self, procurement_data=None):
        ...
        self.old_values = {
            self.warehouse_id: ("resupply_wh_ids", self.warehouse_id.resupply_wh_ids),
            self.warehouse_id4: ("resupply_wh_ids", self.warehouse_id4.resupply_wh_ids),
        ...
        with odoo.api.Environment.manage():
            with odoo.registry(self.env.cr.dbname).cursor() as newcr:
                newenv = api.Environment(newcr, self.env.uid, self.env.context)
                # Add resupply warehouse to warehouse of test & location id
                # to reordering rule
                self.warehouse_id.with_env(newenv).write({"resupply_wh_ids": [(6, 0, [self.yourcompany_id.id])]})
                self.warehouse_id4.with_env(newenv).write({"resupply_wh_ids": [(6, 0, [self.yourcompany_id.id])]})
                self.wh_op.with_env(newenv).write({"location_id": self.warehouse_id.lot_stock_id.id})
                self.wh_op4.with_env(newenv).write({"location_id": self.warehouse_id4.lot_stock_id.id})
                ...
                self.product_id.with_env(newenv).write({"route_ids": [(6, 0, stock_lot.ids + stock_lot4.ids)]})
                newenv.cr.commit()
    ...
    def debug_info(self):
        ...
        """
        wh_fields = ["name", "resupply_wh_ids", "lot_stock_id", "resupply_from_wh"]
        pc_fields = ["name", "route_ids", "seller_ids", "qty_available"]
        proc_fields = ["id", "name", "location_id", "warehouse_id", "rule_id"]
        prorule_fields = ["id", "name", "route_id", "procure_method", "action", "sequence"]
        # compiling debug data for procurement order
        po_all = self.proc.search([("warehouse_id", "=", self.warehouse_id.id)])
        ...
        wh_all = self.warehouse.search([("name", "=", "Test warehouse 3")])
        wh_return = wh_all.read(wh_fields)

        # compiling debug data for procurement rule
        pr_all = self.procurement_rule.search([("warehouse_id", "=", self.warehouse_id.id)])
        pr_return = pr_all.read(prorule_fields)

        stock_lot_route = self.stock_route.search([("supplied_wh_id", "=", self.warehouse_id.id)])
    ...
    def test_10_validate_yourcompany_supply_tw3(self):
        ...
        dom = [("warehouse_id", "=", self.warehouse_id.id), ("product_id", "=", self.product_id.id)]
        self.reset_data(dom)
    ...
    def test_20_validate_importance_supply_tw3(self):
        ...
        dom = [("product_id", "=", self.product_id.id), ("warehouse_id", "=", self.warehouse_id4.id)]
        self.reset_data(dom)
        self.write_data({"importance": self.importance})
        procurement = self.proc.search(dom, limit=1)
        order_point_srch = order_point_obj.search(
            self.cr,
            self.uid,
            [

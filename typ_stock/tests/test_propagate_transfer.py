...
class TestPropagateTransfer(TestTypStock):
    def test_10_transfer_all_pickings(self):
        ...
        self.first_pick.move_lines[0].move_dest_id.rule_id.write({"propagate_transfer": True})
        self.first_pick.move_lines[0].move_dest_id.move_dest_id.rule_id.write({"propagate_transfer": True})

        self.wizard_transfer_id.do_detailed_transfer()

        # 3 pickings it must have passed to done state
        list_states = [picking.state for picking in self.sale_order.picking_ids]

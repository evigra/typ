...
class TestTypAccount(common.TransactionCase):
    def setUp(self):
        ...
        self.partner = self.env.ref("typ.partner_01")
        self.warehouse = self.env.ref("typ.wh_01")
        self.warehouse_2 = self.env.ref("typ_stock.whr_test_01")
        self.journal = self.journalrec = self.env["account.journal"].search([("type", "=", "sale")], limit=1)
        self.journal_bank = self.env["account.journal"].create({"name": "Bank", "type": "bank", "code": "BNK67"})
        self.sale_team = self.env["crm.team"].create(
            {
                "name": "Sale team 01",
                "default_warehouse": self.warehouse.id,
                "journal_team_ids": [(6, 0, [self.journal_bank.id, self.journal.id])],
            }
        )
        ...
        self.payment_term_credit = self.env.ref("payment_term_type.payment_term_credit")
        self.payment_term_cash = self.env.ref("account.account_payment_term_immediate")
        self.conf_warehouse = self.env.ref("typ.res_partner_wh_01")
        ...
        self.sale_order = self.sale_order_model.create(self.dict_vals_sale.copy())

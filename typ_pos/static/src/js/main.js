odoo.define("typ_pos.pos_stock", function (require) {
    ...
    (user_model.loaded = function (self, users) {
        ...
        (product_model.context = function (self) {
            return {location: self.config.stock_location_id[0]};
        }),
        (product_model.loaded = function (self, products) {
            var using_company_currency =
                self.config.currency_id[0] === self.company.currency_id[0];
            ...
            if (self.config.wk_hide_out_of_stock) {
                ...
                for (var i = 0, len = products.length; i < len; i++) {
                    switch (self.config.wk_stock_type) {
                        case "forecasted_qty":
                            if (
                                products[i].virtual_available > 0 ||
                                products[i].type == "service"
                            )
                                available_product.push(products[i]);
                            break;
                        case "virtual_qty":
                            if (
                                products[i].qty_available - products[i].outgoing_qty >
                                    0 ||
                                products[i].type == "service"
                            )
                                available_product.push(products[i]);
    ...
    models.Order = models.Order.extend({
        ...
        add_product: function (product, options) {
            ...
            var stockChecks = [
                ...
                tagExists,
                parseInt($("#qty-tag" + product.id).html()) <=
            ...
            if (partner && partner.pricelist_ids.length) {
                for (var i = 0, len = pricelists.length; i < len; i++) {
                    if (!partner.pricelist_ids.includes(self.pos.pricelists[i].id)) {
                        pricelists.splice(
                            pricelists.indexOf(self.pos.pricelists[i]),
                            1
                        );
                    }
                }
    ...
    screens.NumpadWidget.include({
        start: function () {
            ...
            this.$el
                ...
                .click(_.bind(this.clickDeleteLastChar, this));
    ...
    models.NumpadState = models.NumpadState.extend({
        delete_last_char_of_buffer: function () {
            if (this.get("buffer") === "") {
                ...
                else this.trigger("set_value", this.get("buffer"));
            } else {
    ...
    models.Orderline = models.Orderline.extend({
        ...
        set_quantity: function (quantity) {
            ...
            if (
                ...
                isNaN(quantity) != true &&
                quantity != "" &&
                parseFloat(self.comment) - parseFloat(quantity) <
                    self.pos.config.wk_deny_val &&
                self.comment != 0.0
            ) {
                self.pos.gui.show_popup("error", {
                    ...
                    body: _t(
                        "(" +
                            this.option.product.display_name +
                            ")" +
                            self.pos.config.wk_error_msg +
                            "."
                    ),
                ...
                if (self.pos.get("selectedOrder")) {
                    var wk_pro_order_line = self.pos
                        .get("selectedOrder")
                    ...
                    if (
                        ...
                        var wk_current_qty = parseInt(
                            $("#qty-tag" + wk_pro_order_line.product.id).html()
                        );
                        if (quantity == "" || quantity == "remove") {
                            wk_avail_pro = wk_current_qty + wk_pro_order_line;
                        } else {
                            wk_avail_pro =
                                wk_current_qty + wk_pro_order_line - quantity;
                        }

                        if (
                            wk_avail_pro < self.pos.config.wk_deny_val &&
                            ...
                            self.pos.gui.show_popup("error", {
                                ...
                                body: _t(
                                    "(" +
                                        wk_pro_order_line.product.display_name +
                                        ") " +
                                        self.pos.config.wk_error_msg +
                                        "."
                                ),

...
class TestPosSaleTeam(TestPointOfSaleCommon):
    def test_10_saleteam_invoice(self):
        ...
        self.pos_config.write({"crm_team_id": self.team.id})
        ...
        pos_payment = self.pos_make_payment.with_context(context_make_payment).create({"amount": (450 * 2 + 300 * 3)})
        ...
        self.assertEqual(invoice.team_id, self.team, "Session not configured with the sales team")

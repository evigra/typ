...
class StockBarcodeNotracking(models.TransientModel):
    ...
    stock_barcode_product_line_ids = fields.One2many("stock.barcode.product.line", "stock_barcode_product_id")
    ...
    def default_get(self, fields_list):
        ...
        if "stock_barcode_product_line_ids" in fields_list and candidates_ids:
            ...
            for ml in candidates:
                qty_done = ml.qty_done + 1 if ml.qty_done < ml.product_uom_qty else ml.qty_done
    ...
    def onchange_quantity_done(self):
        ...
        suitable_lines = self.stock_barcode_product_line_ids.filtered(lambda l: l.line_product_barcode == barcode)
        if len(suitable_lines) > 1:
            list_diff = suitable_lines.filtered(lambda x: x.qty_done - x.qty_reserved != 0).mapped(
        ...
        qty_done = self.qty_done if self.qty_done != 0 else suitable_lines.qty_done
        suitable_lines.qty_done = qty_done
        if suitable_lines.qty_done > suitable_lines.qty_reserved:
            raise UserError(_("You cannot fill more than the requested quantity"))
    ...
    def on_barcode_scanned(self, barcode):
        ...
        if not suitable_lines:
            raise UserError(_("Be sure that you are scanning the same product"))
    ...
    def validate_product_set(self):
        self.ensure_one()
        for line in self.stock_barcode_product_line_ids.filtered("line_product_barcode"):
            vals = {}

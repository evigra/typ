    ...
    def on_barcode_scanned(self, barcode):
        ...
        if picking_type.code != "incoming" and product.tracking != "none":
            lot = self.env["stock.production.lot"].search(
                [("product_id", "=", product.id), ("name", "=", barcode)], limit=1
            )
            if not lot and not picking_type.use_create_lots and picking_type.use_existing_lots:
                raise UserError(_("Scanned number it's not a valid " "Serial number for this product"))
        already_scanned = self.stock_barcode_lot_line_ids.filtered(lambda l: l.lot_name == barcode and l.qty_done > 0)
        ...
        already_exist = self.stock_barcode_lot_line_ids.filtered(
            lambda l: l.move_line_id.lot_id.name == barcode and l.qty_done == 0
        )

        suitable_line = self.stock_barcode_lot_line_ids.filtered(lambda l: not l.lot_name and l.qty_done == 0)
        if already_exist:
            suitable_line = already_exist
        if not suitable_line:
            raise UserError(_("You can't add more products than the reserved."))
        vals = {}
    ...
    def default_get(self, fields):
        ...
        if res.get("stock_barcode_lot_line_ids"):
            for values in res["stock_barcode_lot_line_ids"]:
                values[2]["lot_name"] = values[2]["lot_name"] if values[2]["qty_done"] > 0 else False
        return res

    def validate_lot(self):
        res = super().validate_lot()

...
class TestSeparatePurchaseFromProcurement(TransactionCase):
    def setUp(self):
        ...
        self.route_buy.pull_ids.write({"group_propagation_option": "propagate"})
        ...
        self.sale_order_1 = self.env["sale.order"].create(self.dict_vals.copy())
    ...
    def test_00_no_join_purchase_orders(self):
        ...
        """
        purchase_order = self.env["purchase.order"].search([("origin", "=", self.sale_order_1.name)])
        ...
        self.assertTrue(self.env["purchase.order"].search([("origin", "=", sale_order.name)]))

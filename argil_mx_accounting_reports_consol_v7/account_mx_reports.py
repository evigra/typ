    ...
    def button_get_info(self, cr, uid, ids, context=None):
        ...
        for params in self.browse(cr, uid, ids):
            ...
            BEGIN
                ...
                select                 
                    ...
                    and line.period_id in (select period_ids2.id from period_ids2)
                                        --(select xperiodo.id from account_period xperiodo 
                                        --where xperiodo.fiscalyear_id in _period_fiscalyear
            ...
            sql2 = """
	            ...
	            select move_line.id as id, move.name as name, move.date move_date, move.name move_name, move.ref move_ref, 
	            ...
	            from account_move move--, account_period period
                    ...
                    inner join account_period period on move_line.period_id=period.id and period.id in (select period_ids.id from period_ids)
                                                                                      --name >= _period_name_start
		                                                                              --and period.name  <= _period_name_stop 
                                                                                      --and period.fiscalyear_id = _period_fiscalyear
	            where 
	            move.state='posted'
...
class account_mx_report_data_wizard(osv.osv_memory):
    ...
    def get_info(self, cr, uid, ids, context=None):
        ...
        for params in self.browse(cr, uid, ids, context=context):
            ...
            --- Copiamos la tabla account_move_line con los movimientos del periodo de la balanza.
            --RAISE NOTICE 'Copiamos los datos de account_move_line en otra tabla';
            drop table if exists argil_account_move_line;
    
            create table argil_account_move_line as
            select id, move_id, account_id, period_id, journal_id, state, partner_id, debit, credit
            from account_move_line
            where period_id in (select id from account_period 
                     where substring(name,4,4)=(select substring(name, 4,4) from account_period where id=""" + str(params.period_id.id) + """) 
                     and name <= (select name from account_period where id=""" + str(params.period_id.id) + """)
                     );

        --    RAISE NOTICE 'Fin de copia, ahora creando indices...' ;   
            --create index account_move_line_index1 on argil_account_move_line(account_id, period_id, state, move_id);
            create index argil_account_move_line_index2 on argil_account_move_line(account_id, period_id, state, move_id, partner_id);
         --   RAISE NOTICE 'Fin de creacion de indices...';
            ...
            BEGIN
                ...
                case date_part('month', period.date_start)
                    ...
                    account_type.sign * 
                    (select COALESCE(sum(line.debit), 0.00) -  COALESCE(sum(line.credit), 0.00)
                    from account_move move, argil_account_move_line line, account_journal journal
                ...
                move_line.amount_currency::float amount_currency,
                move_line.currency_id
                from account_move move, argil_account_move_line move_line, account_period period, account_fiscalyear fiscalyear,

...
class InvoiceFromGuides(models.TransientModel):
    ...
    def validate_guides(self, partner, currency):
        ...
        # be 1
        if len(partner) != 1:
            raise exceptions.ValidationError(_("All guides selected must have" " the same partner"))
        # All guides must have the same currency, therefore len of currency
        # must be 1
        if len(currency) != 1:
            raise exceptions.ValidationError(_("All guides selected must have" " the same currency"))
    ...
    def create_invoice(self):
        ...
        if is_invoiced:
            ...
            raise exceptions.ValidationError((msg + _("is already invoiced")) % tuple(names))

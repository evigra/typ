...
class StockLandedGuides(models.Model):
    ...
    company_id = fields.Many2one(
        ...
        default=lambda self: self._get_user_default_company(),
    ...
    currency_id = fields.Many2one(
        ...
        readonly=True,
        states={"draft": [("readonly", False)]},
        default=lambda self: self._get_user_default_currency(),
    ...
    state = fields.Selection(
        [("draft", "Draft"), ("valid", "Valid"), ("cancel", "Cancelled")],
        string="Status",
        default="draft",
        help="Guide status",
    )
    ...
    invoiced = fields.Boolean(help="When the guide has been invoiced, this" " field is True, otherwise False")
    ...
    def action_move_create(self):
        ...
        for guide_brw in self:
            if not guide_brw.journal_id.sequence_id:
                raise except_orm(_("Error!"), _("Please define sequence on the journal related to this" " guide."))
            if not guide_brw.line_ids:
                raise except_orm(_("No Guide Lines!"), _("Please create some guide lines."))
            ...
            if not guide_brw.date:
                guide_brw.with_context(ctx).write({"date": fields.Date.context_today(self)})
            date = guide_brw.date
            ...
            gml = guide_brw.with_context(ctx).compute_guide_totals(company_currency, ref, gml)[2]

            part = self.env["res.partner"]._find_accounting_partner(guide_brw.partner_id)
...
class AccountInvoiceLine(models.Model):
    ...
    guide_line_id = fields.Many2one("stock.landed.cost.guide.line", help="Guide line associated to this" " invoice")

    @api.model
    def move_line_get_item(self, line):
        res = super().move_line_get_item(line)

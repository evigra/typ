...
class StockPicking(models.Model):
    ...
    def action_cancel_sale(self):
        ...
        if (
            self._context.get("bypass_check_sale")
            or not sale
            or not all(picking.state == "cancel" for picking in sale.picking_ids)
        ):
            return False
        self.check_access_rights("write")
        self.check_access_rule("write")
        return sale.with_context(bypass_check_sale=True).action_cancel()
    ...
    def action_cancel(self):
        ...
        message = "%s\n<ul>%s</ul>" % (
            _("The following transfers have been cancelled:"),
            "".join(
                "<li><a href=# data-oe-model=stock.picking data-oe-id=%d>%s</a></li>" % (pick.id, pick.name)
                for pick in self
            ),
        )

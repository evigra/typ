...
class SaleOrderLine(models.Model):
    ...
    def _get_purchase_partner_id(self):
        suppliers = self.env["product.supplierinfo"].search_read([("product_id", "=", self.product_id.id)], ["name"])
        return [("id", "in", [x["name"][0] for x in suppliers])]

    purchase_partner_id = fields.Many2one(
        "res.partner",
        string="Supplier for purchase",
    ...
    def _compute_is_special_sale(self):
        for rec in self:
            pull_buy = rec.route_id.pull_ids.filtered(lambda dat: dat.action == "buy")
    ...
    def check_margin_qty(self, price_subtotal=False):
        ...
        can_sell_bellow_minimun_margin = self.env.user.has_group("typ.res_group_can_sell_below_minimum_margin")
        if (
            self.product_id.type != "product"
            or self.order_id.state in ["sale", "cancel"]
            or can_sell_bellow_minimun_margin
        ):
            return False
        # import pdb; pdb.set_trace()
        res = {"message": _("The product [%s] %s %s can not be sold below the allowed " "margin\nContact Manager") % (
            self.product_id.default_code, self.product_id.name, self.product_id.display_name)}
    ...
    def _onchange_price_unit(self):
        ...
        if self.env.user.has_group("typ.res_group_modify_price_sale") or not (
        ...
        if (
            ...
            and self.price_unit != tmp_price_unit
            and not self.product_id.categ_id.allow_change_price_sale
        ):
            raise ValidationError(_("You can not modify the price sale of product %s") % (self.product_id.name))
...
#             suppliers = move.product_id.seller_ids.search([("name", "in", partner_id.ids)])

import mock
from odoo.addons.typ_landed_costs.tests import common
...
class TestAdjustExchangeDifferential(common.TestTypLandedCosts):
    def setUp(self):
        ...
        self.prod = self.env.ref(
            "typ_landed_costs." "product_adjusts_landed_cost_1"
        )
        self.adjust_purchase = self.env.ref(
            "typ_landed_costs." "adjust_purchase_landed_1"
        )
    ...
    def get_landed_values(self, diff, picking, invoice):
        ...
        product = self.env.ref(
            "typ_landed_costs." "landed_exchange_differential_product"
        )
        journal = self.env.ref("stock_account.stock_journal").id
        account = (
            diff > 0
            and self.env.user.company_id.income_currency_exchange_account_id
            or self.env.user.company_id.expense_currency_exchange_account_id
        )
        cost_line = {
            ...
            "price_unit": diff,
            "segmentation_cost": (
                diff == 100 and "landed_cost" or "material_cost"
            ),
        }
        values = {
    ...
    def create_picking_and_invoice(self):
        ...
        self.assertTrue(
            self.adjust_purchase.picking_ids,
            "The purchase order does not have pickings",
        )
        pick = self.adjust_purchase.picking_ids.filtered(
        ...
        # Create invoice
        value = self.stock_invoice.with_context(
            {
                "active_model": "stock.picking",
                "active_id": pick.id,
                "active_ids": [pick.id],
        ...
        invo_id = self.stock_invoice.with_context(
            {
                "active_model": "stock.picking",
                "active_id": pick.id,
                "active_ids": [pick.id],
        ...
        invoice_ids = invo_id.with_context(
            {
                "active_model": "stock.picking",
                "active_id": pick.id,
                "active_ids": [pick.id],
        ...
        self.assertTrue(len(invoice_ids) > 0, "The invoice was not created")
        return pick, invoice_ids

    @mock.patch(
        "odoo.addons.typ_landed_costs.models."
    ...
    def test_00_adjust_from_invoice(self, mock_values):
        ...
        inv_brw = self.env["account.move"].browse(inv_id[0])
    ...
    def test_00_adjust_from_pedimento(self, mock_values):
        ...
        rule = self.env.ref(
            "typ_landed_costs." "automated_action_adjust_exchange_differential"
        )
